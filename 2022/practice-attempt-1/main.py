...
for p in Path("data/").glob("*.in.txt"):
  ...
  if p_name not in {"a_an_example", "b_basic"}:
    ...
    if True:  # that's a lot of printing
      ...
      print("file:", p_name)  # noqa: T201
      print("total potential clients:", count)  # noqa: T201
      print("clients that do like:", len(client_likes))  # noqa: T201
      print("clients that dislike:", len(client_dislikes))  # noqa: T201
      print("clients that like and do not dislike:", len(set(client_likes) - set(client_dislikes)))  # noqa: T201
      print("clients that dislike and do not like:", len(set(client_dislikes) - set(client_likes)))  # noqa: T201
      print("clients that like as well as dislike:", len(set(client_likes) & set(client_dislikes)))  # noqa: T201
      print("clients that neither like or dislike:", count - len(set(client_likes) | set(client_dislikes)))  # noqa: T201
      print("available ingredients:", len(ingredient_pool))  # noqa: T201
      print("liked ingredients:", len(liked_by))  # noqa: T201
      print("disliked ingredients:", len(disliked_by))  # noqa: T201
      print("ingredients that are liked and are not disliked:", len(set(liked_by) - set(disliked_by)))  # noqa: T201
      print("ingredients that are disliked and are not liked:", len(set(disliked_by) - set(liked_by)))  # noqa: T201
      print("ingredients that are liked yet also disliked:", len(set(liked_by) & set(disliked_by)))  # noqa: T201
      print(  # noqa: T201
        "ingredients that are neither liked nor disliked:", len(ingredient_pool) - len(set(liked_by) | set(disliked_by))
      )
      if len(liked_by):
        print("most frequently liked:", lenmax(liked_by), "with", len(liked_by[lenmode(liked_by)]), "likes")  # noqa: T201
      if len(disliked_by):
        print(  # noqa: T201
          "most frequently disliked:", lenmax(disliked_by), "with", len(disliked_by[lenmode(disliked_by)]), "dislikes"
        )
      print("top", min(5, len(liked_by)), "frequently liked:", list(liked_by.keys())[-5:])  # noqa: T201
      print("top", min(5, len(disliked_by)), "frequently disliked:", list(disliked_by.keys())[-5:])  # noqa: T201
      if len(liked_by):
        print("stat. mode liked:", lenmode(liked_by), splat(liked_by).count(lenmode(liked_by)))  # noqa: T201
      if len(disliked_by):
        print("stat. mode disliked:", lenmode(disliked_by), splat(disliked_by).count(lenmode(disliked_by)))  # noqa: T201
    ...
    if len(on_the_pizza) < 10:  # noqa: PLR2004
      print(sorted(on_the_pizza), "scored:", score(on_the_pizza), "/", len(client_likes))  # noqa: T201
    else:
      print("a pizza with", len(on_the_pizza), "ingredients scored:", score(on_the_pizza), "/", len(client_likes))  # noqa: T201
    print()  # noqa: T201
...
print("total score for this run:", SCORE)  # noqa: T201
