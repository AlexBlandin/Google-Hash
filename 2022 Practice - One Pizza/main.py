...
for p in Path("data/").glob("*.in.txt"):
  ...
  if p_name not in ["a_an_example", "b_basic"]:
    ...
    if True: # that's a lot of printing
      ...
      print(
        "ingredients that are neither liked nor disliked:",
        len(ingredient_pool) - len(set(liked_by) | set(disliked_by))
      )
      if len(liked_by):
      ...
      if len(liked_by): print("stat. mode liked:", lenmode(liked_by), splat(liked_by).count(lenmode(liked_by)))
      if len(disliked_by):
    ...
    if len(on_the_pizza) < 10: print(sorted(on_the_pizza), "scored:", score(on_the_pizza), "/", len(client_likes))
    else: print("a pizza with", len(on_the_pizza), "ingredients scored:", score(on_the_pizza), "/", len(client_likes))
    print()
