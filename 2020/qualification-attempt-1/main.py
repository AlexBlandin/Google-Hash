...
def main() -> None:  # noqa: C901, D103
  for path in sorted(Path().glob("*.txt")):
    ...
    print(f"{len(sent)}/{_b} books sent (from {_l - len(queue)}/{_l} libraries)")  # noqa: T201
    print(f"{sum(book_scores[b] for b in sent)} score (vs {pb[name]}) in {time() - starttime:.2f}s")  # noqa: T201
