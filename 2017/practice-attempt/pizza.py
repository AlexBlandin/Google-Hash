...
for path in ["example.in", "small.in", "medium.in", "big.in"][2:3]:
  ...
  print(f"Rows: {rows} rows, {columns} columns, {minimum_ingredients} ingredients minimum, {maximum_area} area maximum")  # noqa: T201
  ...
  if coverable_area != total_area:
    print(f"Only {coverable_area} of {total_area} area is coverable")  # noqa: T201
    ...
    print(f"We can cover all {coverable_area} area")  # noqa: T201
  print(f"Viable area is {viable_area}")  # noqa: T201
  # Ordered, largest slices first
  candidates.sort(key=quadarea, reverse=True)
  candidate_count = len(candidates)
  print(f"{candidate_count} candidates")  # noqa: T201
  ...
  print(f"Solving by {use}")  # noqa: T201
  ...
  print(f"Best Area: {best_area}/{total_area} ({best_area / total_area:.1%})")  # noqa: T201
...
print()  # noqa: T201
print(f"Total score: {total_score}")  # noqa: T201
